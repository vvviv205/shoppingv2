...
export default function Homepage() {
  ...
  return (
    <div className="dark:bg-black h-fit min-h-screen ">
      <div className="flex flex-col-reverse sm:flex-row justify-end sm:justify-between min-h-screen  pt-10 sm:pt-44 mx-10 ">
        ...
        <center>
          ...
          <div className="flex flex-wrap justify-center gap-4 mt-7">
            {Categories.map((c) => (
              <NavLink
                to={`/products/${c}`}
                className="text-white text-base sm:text-xl p-4 bg-neutral-900 rounded-xl cursor-pointer"
              >
                {c.toUpperCase()}
              </NavLink>
            ))}
